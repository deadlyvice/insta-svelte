...
{#if isOpen}
  ...
  <div class="overlay" role="dialog" tabindex="1" aria-modal="true" on:click={() => dispatch('close')}>
    ...
    <div class="modal" on:click|stopPropagation>
      ...
      <form on:submit|preventDefault={submit} class="modal-body">
        ...
        <label>
          Content
          <textarea bind:value={content} placeholder="Write your post..." rows="6"></textarea>
        </label>

        <label>
          Images (comma-separated URLs)
          <input type="text" bind:value={images} placeholder="https://... , https://..." />
        </label>
...
<style>
  ...
  @media (prefers-color-scheme: dark) {
    .modal { background: #071021; color: #e6eef8; }
  }

  .modal-header h3 { margin: 0 0 8px 0; font-size: 1.125rem; }
  ...
  .error { color: #b91c1c; font-size: 0.9rem; padding-top:4px; }
</style>